...
function configureHeader(){
  document.getElementById("organizationName").textContent=config.organizationName||"INSTANSI";
  document.getElementById("weatherTemp").textContent=config.weatherTemperature||"";
  document.getElementById("weatherText").textContent=config.weatherDescription||"";
  document.getElementById("weatherIcon").textContent=config.weatherIcon||"☀️";
}
...
function renderCalendar(){
  ...
  document.getElementById("calendarMonth").textContent=new Intl.DateTimeFormat("id-ID",{month:"long",year:"numeric"}).format(now);
  const first=new Date(y,m,1),start=first.getDay(),days=new Date(y,m+1,0).getDate(),prev=new Date(y,m,0).getDate(),cells=[];
  for(let i=start-1;i>=0;i--)cells.push({n:prev-i,muted:true,date:new Date(y,m-1,prev-i)});
  for(let d=1;d<=days;d++)cells.push({n:d,date:new Date(y,m,d)});
  let n=1;while(cells.length<42)cells.push({n:n,muted:true,date:new Date(y,m+1,n++)});
  const events=new Set(allData.map(x=>x.tanggal));
  document.getElementById("calendarGrid").innerHTML=cells.map(c=>{const iso=toLocalISO(c.date);return `<div class="day ${c.muted?'muted':''} ${iso===todayISO()?'today':''} ${events.has(iso)?'has-event':''}">${c.n}</div>`}).join('');
}
function renderChart(items){
  const order=['outside','office','meeting','monitoring','leave'];
  const labels={outside:'Tugas Luar',office:'Kantor',meeting:'Rapat',monitoring:'Monitoring',leave:'Cuti/Libur'};
  const colors={outside:'#175cd3',office:'#18a957',meeting:'#f28c28',monitoring:'#7a5af8',leave:'#d64545'};
  const counts={outside:0,office:0,meeting:0,monitoring:0,leave:0};items.forEach(x=>counts[statusClass(x.status)]++);
  const total=items.length;document.getElementById('chartTotal').textContent=total;let a=0,parts=[];
  order.forEach(k=>{const d=total?counts[k]/total*360:0;parts.push(`${colors[k]} ${a}deg ${a+d}deg`);a+=d});
  document.getElementById('donutChart').style.background=total?`conic-gradient(${parts.join(',')})`:'conic-gradient(#dfe6ef 0deg 360deg)';
  document.getElementById('chartLegend').innerHTML=order.map(k=>`<div class="legend-row"><div class="legend-left"><span class="legend-color" style="background:${colors[k]}"></span>${labels[k]}</div><span class="legend-value">${counts[k]}</span></div>`).join('');
}
